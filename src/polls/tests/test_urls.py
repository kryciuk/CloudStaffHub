...
class TestUrls(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.poll_answer = PollAnswer(respondent=self.user_owner, date_filled="2030-01-01", poll=self.poll, result={})
        self.poll_answer.save()
        self.poll_results = PollResults(poll=self.poll, results={}, close_date="2030-10-10")
        self.poll_results.save()
        ic(self.poll.questionnaire.company)
        ic(self.user_owner.profile.company)
        self.urls = {
