...
class TestEmployeeUrls(TestCase):
    ...
    def test_url_exists_at_correct_location_if_user_has_group_employee(self):
        ...
        response = self.client.get('/employee/', {'user_id': self.user.id})
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_url_available_by_name_if_user_has_group_employee(self):
        user_login = self.client.login(username=self.user.username, password='password')
        response = self.client.get(reverse('employee-dashboard'), {'user_id': self.user.id})
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_url_forbidden_if_user_is_not_employee(self):
        user_login = self.client.login(username=self.user_not_employee.username, password='password')
        response = self.client.get(reverse('employee-dashboard'), {'user_id': self.user_not_employee.id})
